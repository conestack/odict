...
from odict.codict import _nil
from odict.codict import _codict
import copy
import pickle
import sys
import unittest
...
class TestCodict(unittest.TestCase):
    ...
    def test_init_with_kw(self):
        ...
        self.assertRaisesWithMessage(msg, codict, TypeError, a=1)

    def test_init_with_dict(self):
        # When initialized with a dict instance, the order of elements is
        # undefined.
        o = codict({'b': 2, 'a': 1, 'd': 4, 'c': 3})
    ...
    def test_delete(self):
        ...
        # Removal from codict with one element
        o['1'] = 1
        del o['1']
        self.assertEqual([o.lh, o.lt, o], [_nil, _nil, codict()])
        self.assertEqual(o._repr(), 'codict low level repr lh,lt,data: nil, nil, {}')
        # Remove first element of the codict sequence
        o = codict([('1', 1), ('2', 2), ('3', 3)])
        del o['1']
        self.assertEqual([o.lh, o.lt, o], ['2', '3', codict([('2', 2), ('3', 3)])])
        # Remove element in the middle of the codict sequence
        o = codict([('1', 1), ('2', 2), ('3', 3)])
        del o['2']
        self.assertEqual([o.lh, o.lt, o], ['1', '3', codict([('1', 1), ('3', 3)])])
        # Remove element at the end of the codict sequence
        o = codict([('1', 1), ('2', 2), ('3', 3)])
        del o['3']
        self.assertEqual(
            [o.lh, o.lt, o],
    ...
    def test_casting(self):
        ...
        # pickle.
        # Note: codict uses C structs so the behavior might differ from odict
        self.assertEqual(dict(codict([(1, 1)]).items()), {1: 1})
        self.assertEqual(codict([(1, 1)]).as_dict(), {1: 1})

    def test_pickle(self):
        # dump and load codict pickle
        self.assertEqual(
            pickle.loads(pickle.dumps([codict([(1, 2)])])), [codict([(1, 2)])]
        )

    def test_pickle_roundtrip(self):
        # Additional test: pickle/unpickle roundtrip with C structs
        o = codict([('a', 1), ('b', 2), ('c', 3)])
        pickled = pickle.dumps(o)
        unpickled = pickle.loads(pickled)
        self.assertEqual(unpickled.items(), [('a', 1), ('b', 2), ('c', 3)])
        self.assertEqual(unpickled.keys(), ['a', 'b', 'c'])
        self.assertEqual(unpickled.values(), [1, 2, 3])

    def test_sort(self):
        # basic sorting
        o = codict([('a', 1), ('c', 3), ('b', 2)])
        o.sort()
    ...
    def test_bool_expessions(self):
        ...
        self.assertFalse(codict() and True or False)
        self.assertTrue(codict([('a', 1)]) and True or False)
        self.assertTrue(bool(codict([('a', 1)])))

    def test_alter_key(self):
        # Test alter_key function.
        o = codict((('1', 'a'), ('2', 'b'), ('3', 'c')))
    ...
    def test_str(self):
        ...
        o = codict([('foo', 'a'), ('bar', 'b'), ('baz', 'c')])
        self.assertEqual(str(o), "{'foo': 'a', 'bar': 'b', 'baz': 'c'}")

    def test_repr(self):
        self.assertEqual(repr(codict()), 'codict()')
        self.assertEqual(repr(codict([('foo', 'a')])), "codict([('foo', 'a')])")

    def test_swap(self):
        # Test ``swap``
        o = codict([('0', 'a'), ('1', 'b'), ('2', 'c'), ('3', 'd'), ('4', 'e')])
